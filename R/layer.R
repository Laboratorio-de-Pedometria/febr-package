    ...
    for (i in 1:length(sheets_keys$camada)) {
      ...
      if (missing$depth == "keep" || missing$depth == "drop" && na_depth < n_rows) {
        ...
        if (n_rows >= 1 && missing(variable) || 
          ...
          # TEMP: corrigir valores fora do intervalo real #####################################################
          # pH
          ph <- grep("^ph_", cols)
          if (length(ph) >= 1) {
            ph_idx <- which(tmp[, ph] > 1000, arr.ind = TRUE)
            if (length(ph_idx) >= 1) {
              tmp[, ph][ph_idx] <- tmp[, ph][ph_idx] / 100
            }
            ph_idx <- which(tmp[, ph] > 100, arr.ind = TRUE)
            if (length(ph_idx) >= 1) {
              tmp[, ph][ph_idx] <- tmp[, ph][ph_idx] / 100
            }
            ph_idx <- which(tmp[, ph] > 14, arr.ind = TRUE)
            if (length(ph_idx) >= 1) {
              tmp[, ph][ph_idx] <- tmp[, ph][ph_idx] / 10
            }
          }
          # Profundidade
          profund <- grep("^profund_", cols)
          profund_idx <- which(tmp[, profund] == 9999, arr.ind = TRUE)
          tmp[, profund][profund_idx] <- NA_real_
          profund_idx <- which(tmp[, profund] == 999, arr.ind = TRUE)
          tmp[, profund][profund_idx] <- NA_real_
          # Argila
          argila <- grep("^argila_", cols)
          if (length(argila) >= 1) {
            argila_idx <- which(tmp[, argila] > 1000, arr.ind = TRUE)
            if (length(argila_idx) >= 1) {
              tmp[, argila][argila_idx] <- tmp[, argila][argila_idx] / 10
            }
          }
          # Areia
          areia <- grep("^areia", cols)
          if (length(areia) >= 1) {
            areia_idx <- which(tmp[, areia] > 10000, arr.ind = TRUE)
            if (length(areia_idx) >= 1) {
              tmp[, areia][areia_idx] <- tmp[, areia][areia_idx] / 100
            }
            areia_idx <- which(tmp[, areia] > 1000, arr.ind = TRUE)
            if (length(areia_idx) >= 1) {
              tmp[, areia][areia_idx] <- tmp[, areia][areia_idx] / 10
            }
          }
          # Silte
          silte <- grep("^silte_", cols)
          if (length(silte) >= 1) {
            silte_idx <- which(tmp[, silte] > 1000, arr.ind = TRUE)
            if (length(silte_idx) >= 1) {
              tmp[, silte][silte_idx] <- tmp[, silte][silte_idx] / 10
            }
          }
          # DSI
          dsi <- grep("^dsi_", cols)
          if (length(dsi) >= 1) {
            dsi_idx <- which(tmp[, dsi] > 30, arr.ind = TRUE)
            if (length(dsi_idx) >= 1) {
              tmp[, dsi][dsi_idx] <- tmp[, dsi][dsi_idx] / 100
            }
            dsi_idx <- which(tmp[, dsi] > 3, arr.ind = TRUE)
            if (length(dsi_idx) >= 1) {
              tmp[, dsi][dsi_idx] <- tmp[, dsi][dsi_idx] / 10
            }
          }
          #####################################################################################################
