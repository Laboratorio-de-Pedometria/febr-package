...
date: "20 Fevereiro 2018"
output:
  bookdown::html_document2:
    toc: true
    theme: united
vignette: >
  %\VignetteIndexEntry{Repositório Brasileiro Livre para Dados Abertos do Solo}
...
O pacote `febr` possui três grupos de funções (Tabela \@ref(tab:functions)). As funções estruturais são usadas para descarregar informações sobre as tabelas de dados e os padrões usados no `r febr`. As funções de acesso são usadas para descarregar dados dos conjuntos de dados publicados no `r febr`. Já as funções auxiliares servem de apoio ao processamento e uso dos dados.
...
rbind(
  ...
  pandoc.table(
    caption = "(\\#tab:functions) Descrição sumária das funções do pacote `febr`.", 
...
```{r, results='asis'}
standard(variable = c("argila", "densidade", "carbono", "ph")) %>%
...
```{r}
unit(source = "Mg/m^3", target = "kg/dm^3")
```
...
As funções de acesso servem para efetivamente descarregar os dados dos conjuntos de dados publicados no `r febr`.
...
```{r, results='asis'}
dataset(dataset = "ctb0003", progress = FALSE, verbose = FALSE) %>%
  na.omit() %>%
  pandoc.table(row.names = FALSE, caption = "Dados sobre o conjunto de dados `ctb0003` contidos na tabela  `dataset`.")
```
...
```{r, results='asis'}
set.seed(2001)
metadata(dataset = "ctb0003", progress = FALSE, verbose = FALSE) %>%
  dplyr::select(dataset_id, tabela_id, campo_id, campo_nome, campo_descricao) %>% 
  group_by(tabela_id) %>%
  sample_n(5) %>%
  pandoc.table(row.names = FALSE, 
               caption = "Alguns dos dados sobre os dados contidos no conjunto de dados `ctb0003`.")
```
...
```{r}
header(dataset = "ctb0003", table = "observacao", variable = "all",
       progress = FALSE, verbose = FALSE) %>%
  t()
```
...
```{r}
set.seed(2001)
observation(dataset = c("ctb0003", "ctb0036"),
            stack = TRUE,
            missing = list(coord = "drop", time = "drop"),
...
```{r}
header(dataset = "ctb0036", table = "observacao",
       variable = "all",
       progress = FALSE, verbose = FALSE) %>%
  t()
```
...
```{r}
lyr <- 
  febr::layer(dataset = c("ctb0003", "ctb0036"),
