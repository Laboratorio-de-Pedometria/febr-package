...
A função `febr2sf` serve para criar um objeto do tipo _simple feature_  [@sp] a partir da tabela `observacao` descarregada usando a função `observation`. O código a seguir demonstra como fazer isso -- note que as funções do pacote `febr` são compatíveis com o uso do operador `%>%` do pacote `magrittr` [@magrittr]:
